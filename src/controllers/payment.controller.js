...
import { initiatePayment as initiatePaymentService } from "../services/payment.service.js";
...
export const atomRedirectHandler = async (req, res) => {
  try {
    ...
    if (!txnId) {
      ...
      return res.redirect(
        `${env.FRONTEND_URL}/payment/failure?error=no_txn_id`,
      );
    }
...
export const handlePaymentCallback = async (req, res) => {
  try {
    ...
    let receivedSignature =
      payDetails.signature || payDetails.atomSignature || extras?.signature;
    ...
    if (statusCode === "OTS0000") {
      ...
      if (receivedSignature) {
        ...
        console.warn(
          "No signature received from Atom gateway - proceeding without verification",
        );
      }
