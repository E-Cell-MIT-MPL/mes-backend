...
const getRole = (user) => {
  if (
    user?.learnerEmail &&
    user.learnerEmail.includes("@learner.manipal.edu")
  ) {
    return "MAHE STUDENT";
  }
  return "NON-MAHE";
...
export const scanTicket = async (req, res) => {
  try {
    ...
    if (!decryptedData || !decryptedData.t) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid or tampered QR" });
    }
    ...
    const alreadyScannedToday =
      ticket.entryHistory &&
      ticket.entryHistory.some((entry) => entry.dateString === today);
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    // Count tickets that have at least one entry for today
    const todayScanCount = await Ticket.countDocuments({
      paymentStatus: "SUCCESS",
      entryHistory: { $elemMatch: { dateString: today } },
    });

    // 3. Pending
    const pendingCount = totalSold - todayScanCount;
    ...
    const recentEntries = recentTickets.map((t) => {
      ...
      return {
        ...
        time: new Date(lastEntry.timestamp).toLocaleTimeString("en-IN", {
          timeZone: "Asia/Kolkata",
          hour: "2-digit",
          minute: "2-digit",
        }),
